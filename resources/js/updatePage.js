...
$(document).ready(function(){
    ...
    $(document).on('mousedown touchstart', '.update_history_container_elem', function (event) {
        if(!($(event.target).is('.update_history_bin') || $(event.target).parent().is('.update_history_bin'))){
            let isDragging = false;
            let startX = 0;
            let currentX = 0;
            let deltaX = 0;
            const maxSlide = 75; // Maximum slide distance in pixels
            const swipeThreshold = 10; // Minimum movement to consider as a swipe
            const element = $(this);
        
            if(!$(element).data('swipable')){
                return; 
            };
    
            // Reset transition for smooth sliding
            element.css('transition', 'none');
        
            // Restore any currently swiped element if it's not the same as the new one being swiped
            if (currentlySwipedElement && currentlySwipedElement[0] !== element[0] || $(element).data('swiped')) {
                restoreSwipedElement();
        
                $(currentlySwipedElement).data('swiped', false);
                $(currentlySwipedElement).data('isBeingSwiped', false);
        
                currentlySwipedElement = null;
            }
        
            const startEvent = event.type === 'mousedown' ? event : event.originalEvent.touches[0];
            startX = startEvent.pageX;
        
            function onMouseMove(e) {
                const moveEvent = e.type === 'mousemove' ? e : e.originalEvent.touches[0];
                currentX = moveEvent.pageX;
        
                deltaX = currentX - startX;
        
                // Restrict movement between 0px (original) and -75px (swiped)
                deltaX = Math.max(-maxSlide, Math.min(0, deltaX));
        
                // Apply the sliding effect
                element.css('transform', `translateX(${deltaX}px)`);
                isDragging = Math.abs(deltaX) > swipeThreshold; // Consider it a swipe if threshold exceeded
        
                $(element).data('isBeingSwiped', true);
            }
        
            function onMouseUp() {
                if (isDragging) {
                    // Magnetic behavior: snap to original position or maximum swipe based on swipe distance
                    if (Math.abs(deltaX) >= maxSlide / 2) {
                        // Snap to maximum swipe position
                        element.css({
                            transform: `translateX(-${maxSlide}px)`,
                            transition: 'transform 0.3s ease-out',
                        });
                        currentlySwipedElement = element; // Update the currently swiped element
                        $(currentlySwipedElement).data('swiped', true);
                    } else {
                        // Snap back to the original position
                        currentlySwipedElement = element;
                        restoreSwipedElement();
                    }
                } else {
                    // If no significant drag, return to the original position
                    currentlySwipedElement = element;
                    $(currentlySwipedElement).data('isBeingSwiped', false);
            ...
            // Prevent click on swiped elements and restore their position
            element.off('click').on('click', function (e) {
                if (currentlySwipedElement && currentlySwipedElement[0] === element[0]) {
                    e.preventDefault();
                    e.stopImmediatePropagation();
        
                    // Use the restore function to put the element back
                    if (!isDragging) {
                        restoreSwipedElement();
                    }
                }
            ...
            // Attach event listeners for mouse/touch move and end
            $(document).on('mousemove touchmove', onMouseMove);
            $(document).on('mouseup touchend', onMouseUp);
            
            element.find('.update_history_bin').off('click').on('click', function (e) {
                e.stopPropagation();
                let selectedHistoryIndex = $('.update_history_container_elem').index(element);

                current_history['historyList'] = current_history['historyList'].delete(current_history['historyList'].length - 1 - selectedHistoryIndex);
                $(element).remove();
                ...
                if(current_history['historyList'].length == 0){
                    ...
                    $('.update_history_container').find('update_history_loadMore_btn').remove();
                };
            });
        };
