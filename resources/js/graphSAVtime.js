/* graph.js – version 11.5  (2025-06-24)
 * -----------------------------------------------------------------------
 * Responsive, collision‑free line‑chart — jQuery + Canvas
 * -----------------------------------------------------------------------
 *  NEW IN v11.5
 *  ------------
 *  • Added `getLabel` function parameter for custom Y-value formatting
 *  • Added `integerYaxisLabel` to force integer-only Y-axis ticks
 *  • Y-axis labels and data tags now use getLabel() when provided
 *
 *  PREVIOUS IN v11.4
 *  -----------------
...
(function ($) {
  ...
  function graph(optsIn) {
    ...
    const opts = $.extend({
      ...
      getLabel          : null,   // function to format Y values
      integerYaxisLabel : false,  // force integer Y-axis ticks
      paddings : { 
        t: undefined, 
        b: undefined, 
        l: undefined, 
    ...
    const niceStep = (r, forceInteger = false) => {
      const t = r / 5, p = Math.pow(10, Math.floor(Math.log10(t)));
      const m = t / p;
      let step = (m < 1.5 ? 1 : m < 3 ? 2 : m < 7 ? 5 : 10) * p;
      
      // Force step to be at least 1 if integers are required
      if (forceInteger && step < 1) step = 1;
    ...
    function redraw() {
      ...
      let yAxisWidth = 0;
      if (opts.showYaxe) {
        const st = niceStep(Math.max(...Ynum) - Math.min(...Ynum), opts.integerYaxisLabel) || 0.5;
        const dec = (opts.integerYaxisLabel || st >= 1) ? 0 : Math.min(4, Math.ceil(-Math.log10(st)));
        
        // Calculate max width considering getLabel if provided
        let maxYw;
        if (opts.getLabel) {
          // Sample a few Y values to determine max label width
          const minY = Math.min(...Ynum);
          const maxY = Math.max(...Ynum);
          const sampleValues = [minY, maxY, (minY + maxY) / 2];
          
          maxYw = Math.max(...sampleValues.map(v => {
            const label = opts.getLabel(Math.round(v)); // Use rounded value for width calculation
      ...
      if (n === 1) {
        const xText = isDate
          ...
          : String(X[0]);
        const yText = opts.getLabel ? opts.getLabel(Y[0]) : String(Y[0]);
      ...
      const drawYAxis = (context, xOffset = 0) => {
        ...
        // ticks & labels
        const stepY = niceStep(yMax - yMin, opts.integerYaxisLabel);
        const dec   = (opts.integerYaxisLabel || stepY >= 1) ? 0 : Math.min(4, Math.ceil(-Math.log10(stepY)));
        ...
        for (let v = Math.ceil(yMin / stepY) * stepY; v <= yMax + 1e-9; v += stepY, idx++) {
          ...
          // Use getLabel if provided, otherwise show numeric value
          const labelText = opts.getLabel ? opts.getLabel(Math.round(v)) : v.toFixed(dec);
          context.fillText(labelText, axisX - tickLen - TICK_GAP_Y, yy);
        }
      };
      ...
      if (opts.showDataTag) {
        ...
        for (const i of order) {
          // Use getLabel if provided, otherwise use numeric formatting
          const txt = opts.getLabel 
            ? opts.getLabel(Ynum[i]) 
            : Ynum[i].toFixed(opts.dataTagPrecision);
